...
public class VT_Terrain : MonoBehaviour
{
    public class Node
    {
        ...
        private static int getPhysicIndex()
        {
            return physicEmptyIndexQueue.Count == 0 ? -1 : physicEmptyIndexQueue.Dequeue();
        }

        private static void resetPhysicIndex(Node node)
        {
            if (node.physicTexIndex > -1) physicEmptyIndexQueue.Enqueue(node.physicTexIndex);
        }
        ...
        {
            var dis = CalculateClosestPoint(camPos, new Vector2(x + size / 2, z + size / 2),
    ...
    void Start()
    {
        ...
        clipRTAlbedoArray = new RenderTexture(VirtualCapture.virtualTextArraySize, VirtualCapture.virtualTextArraySize,
        ...
        clipRTNormalArray = new RenderTexture(VirtualCapture.virtualTextArraySize, VirtualCapture.virtualTextArraySize,
    ...
    void OnDrawGizmos()
    {
        ...
        {
            Gizmos.DrawWireCube(terrainOffset + new Vector3(item.x + item.size / 2.0f, 0, item.z + item.size / 2.0f),
                new Vector3(1, 0, 1) * item.size);
            UnityEditor.Handles.Label(
                terrainOffset + new Vector3(item.x + item.size / 2.0f, 0, item.z + item.size / 2.0f),
                item.physicTexIndex + "");
            leafCount++;
        }
    ...
    void Update()
    {
        ...
        Node.updateAllLeavesState(new Vector2(Camera.main.transform.position.x - terrainOffset.x,
    ...
    private void onLoadNodeData(Node item)
    {
        ...
        virtualCapture.virtualCapture_MRT(new Vector2(item.x + item.size / 2.0f, item.z + item.size / 2.0f), item.size,
            out albedoRT, out normalRT);
        for (int i = 0; i < 4; i++)
        {
